...
You only need three things per step: a **video URL**, a **selector**, and a
**time**. To get them without any tooling:

1. **Record** a short narration (Loom, QuickTime, OBS, any screen recorder).
   Export **MP4** and host it (S3, Cloudflare R2, your `/public` folder, a CDN…).
   Put that URL in `videoUrl`.
2. **Selectors** — add `data-onb="…"` to the elements you'll spotlight (most
   reliable), or in devtools right-click the element → *Copy → Copy selector*.
3. **Times** — scrub the video and note the second you mention each thing; set
   `time`. Leave `hold: true` so each spotlight lingers until the next.
...
non-technical teammates to build tours; it writes exactly the JSON shape above.
